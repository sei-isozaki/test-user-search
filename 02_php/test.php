...
foreach ($jkn as $key => $value){
	$sql = "select user_id_csv from user_att2 where att_id =$key  and att_value >= $value ";
        $ids = array();
        foreach($pdo->query($sql) as $row){
             $ids = array_merge($ids, explode(",",$row['user_id_csv']));
        }
        echo count($ids) . ' in condition' . $key. PHP_EOL;
        $result[] = $ids;
}
